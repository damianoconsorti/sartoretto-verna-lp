...
function Field({ label, id, children, required = false, error }: { label: React.ReactNode; id: string; children: React.ReactNode; required?: boolean; error?: string }) {
  return (
...
export default function ContactForm() {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (form.telefono.trim() && (!/^\+?[\d\s().-]{6,}$/.test(form.telefono) || phoneDigits.length < 6)) {
